class Level1 extends Phaser.Scene {
    ...
    create() {
        // Create background
        this.background = this.add.image(400, 300, 'background').setScale(1.15);
        this.door = this.physics.add.image(400, 0, 'door').setScale(0.1);
        ...
        // Create enemy angel
        this.angel = this.physics.add.sprite(400, 200, 'angel').setScale(0.1);
        ...
        // Colliders
        this.physics.add.overlap(this.player, this.angel, this.onPlayerAngelCollision, null, this);
    ...
    update() {
        ...
        const distance = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.angel.x, this.angel.y);

        // If player is within 200 units of angel and not looking at it, move angel towards player
        if (distance < 200 && !this.isPlayerLookingAtEnemy(this.player, this.angel)) {
            this.physics.moveToObject(this.angel, this.player, 100);  // Move towards player
        } else {
            this.angel.setVelocity(0);  // Stop angel if not actively chasing
        }
        if (this.physics.overlap(this.player, this.door)) {
            this.angel.setVelocity(0);  // Stop angel when player reaches the door
        }
    }
